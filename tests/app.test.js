...
jest.unstable_mockModule("../src/middleware/security.middleware.js", () => securityMiddlewareMock);
jest.unstable_mockModule("../src/services/auth.service.js", () => authServiceMock);
jest.unstable_mockModule("../src/services/users.service.js", () => usersServiceMock);
...
const {deleteUser: deleteUserController, fetchAllUsers, getUserById, updateUser} = await import("../src/controllers/users.controller.js");
...
function createControllerRequest({body = undefined, cookies = {}, params = {}} = {}) {
    return {
        body,
        cookies,
        params,
...
async function invokeMiddleware(middleware, req, res = createControllerResponse()) {
...
async function invokeController(controller, req, res = createControllerResponse()) {
    await controller(req, res, () => {});
    return res;
}

beforeEach(() => {
    jwtMock.jwttoken.sign.mockImplementation((payload) => `signed:${payload.id}:${payload.role}`);
...
describe("API endpoints", () => {
    ...
    describe("Auth routes", () => {
        it("POST /api/auth/sign-up returns 201 and sets a token cookie", async () => {
            const response = await invokeController(signup, createControllerRequest({
                body: {
                    name: "John Giotis",
                    email: "john@example.com",
        ...
        it("POST /api/auth/sign-in returns 200 and sets a token cookie", async () => {
            const response = await invokeController(signIn, createControllerRequest({
                body: {
                    email: "john@example.com",
                    password: "password123",
                },
        ...
        it("POST /api/auth/sign-in returns 401 for invalid credentials", async () => {
            authServiceMock.authenticateUser.mockRejectedValueOnce(new Error("Invalid password"));
        ...
        it("POST /api/auth/sign-out clears the token cookie", async () => {
            const response = await invokeController(signOut, createControllerRequest({
                cookies: {
                    token: userToken,
                },
            }));
    ...
    describe("User routes", () => {
        it("GET /api/users requires authentication", async () => {
            const response = await invokeMiddleware(authenticateToken, createControllerRequest());
